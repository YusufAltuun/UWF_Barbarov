...
/* USER CODE BEGIN PV */
char gelen[8];
int degisim,donusum,degisen = 1500;
...
void ilr(int degisim){
	...
	__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, 1500-abs(1500-degisim));
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  else if(gelen[5]=='s' && gelen[6]=='g' && gelen[7]=='d'){
		  sgd(degisim);
	  }
	  else if(gelen[5]=='s' && gelen[6]=='l' && gelen[7]=='d'){
		  sld(degisim);
	  }
  }
  /* USER CODE END 3 */
